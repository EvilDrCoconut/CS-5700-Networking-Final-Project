...
public class DNSClient {
  ...
  public static ArrayList<ResourceRecord> processResponse(DatagramPacket responseSegment)
      throws IOException {
  ...
  void sendQuerySegment(byte[] querySegment, InetAddress hostAddress, int portNum)
      throws IOException {
    DatagramPacket queryPacket = new DatagramPacket(querySegment, querySegment.length,
        hostAddress, portNum);
  ...
  public ArrayList<ResourceRecord> recursiveSearch(String query, String questionType,
    ...
    if (!rrSet.isEmpty() && rrSet != null) {
      ...
      queryDNSServer(this.localServer.getCache().getCacheMap().get("Root").record.getPortNum(),
          InetAddress
              .getByName(this.localServer.getCache().getCacheMap().get("Root").record.getValue()),
          5, 0, 0, 1, tldQuery);
