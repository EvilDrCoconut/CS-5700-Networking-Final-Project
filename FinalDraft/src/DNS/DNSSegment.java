...
public class DNSSegment {
  ...
  private int questionType; // "None"=0, "A"=1, "NS"=2, or "MX"=3 "G"=4, "TLD"=5, "E"=6, 2, or 3
  ...
  static ArrayList<String> readQuestionPayload(byte[] questionPayload) {
    ...
    System.out
        .println("this is the question type from reading the payload " + qStringAndQtype.get(1));
    return qStringAndQtype;
  }

  static byte[] createResponseSegment(byte[] querySegment, ArrayList<ResourceRecord> rrs)
      throws IOException {
    System.out.println("int the create repsonse segment");
    byte[] qHeaderArray = Arrays.copyOfRange(querySegment, 0, 12);
    int[] qHeaderFields = readHeaderFields(readSegmentHeader(querySegment));
    int numRRs = rrs.size(); //rHeaderfields[6];
    int numAuthRRs = rrs.stream().filter(rr -> "A".equals(rr.getType()))
        .collect(Collectors.toList()).size(); // rHeaderFields[7]
    int[] rHeaderFields = new int[]{qHeaderFields[0], 1, qHeaderFields[2], 1, 1, 1, numRRs,
        numAuthRRs, 0};
    ...
    byte[] combinedResponse = new byte[responseSegment.getDnsSegmentHeader().length
        + responseSegment.getPayload().length];
  ...
  static ArrayList<ResourceRecord> readResponsePayload(byte[] responsePayload) throws IOException {
    ...
    try {
      resourceRecords = (ArrayList<ResourceRecord>) recordObject.readObject();
      recordObject.close();
      payloadInput.close();
